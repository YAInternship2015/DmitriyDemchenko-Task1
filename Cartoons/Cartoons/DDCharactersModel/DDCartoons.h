...
@interface DDCartoons : NSObject

//#warning Вы перепутали класс-модель и класс-датасорс. Данный класс - датасорс, он должен хранить массив моделей.
//#warning Показывать наружу (в *.h файле) массив, еще и не readonly - плохая идя. Так любой может его занилить извне. В этом классе-датасорсе внутри (в *.m) должен быть один массив моделей. Также, в *.h файле необходимо показать минимальный интерфейс, который нужен вью контроллеру для работы, то есть нужен метод, который бы возвращал общее количество моделей, и метод, который бы возвращал модель по индексу (для ячейки). И все, массив же показывать не стоит

- (NSArray *)getCartoonCharacters;
